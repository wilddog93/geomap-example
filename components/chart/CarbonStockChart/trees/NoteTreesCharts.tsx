...
interface NoteTreesChartsProps {
  data: CarbonTreesStatisticsProp[];
}

const NoteTreesCharts: FC<NoteTreesChartsProps> = ({ data }) => {
  // color
        ...
        ? regions.map((region) => {
            ...
            return (
              groupedData[key]?.reduce(
                (sum: any, item: any) => sum + item.avg_notes,
                0
              ) || 0
            );
          })
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (item: any) {
            return `${item?.dataset?.label} : ${item?.raw?.toFixed(2)} Notes`
          }
        }
      },
    },
...
export default NoteTreesCharts;
