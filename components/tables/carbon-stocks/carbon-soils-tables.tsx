...
export default function CarbonSoilsTables({
  params,
  page,
  setPage,
  limit,
  ...
  // data-table-with-api
  const { fetch, data, meta, fetching } = useCarbonSoilsApi();
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a: CarbonSoilsTypes, b: CarbonSoilsTypes) => {
      const first = a[
        sortDescriptor.column as keyof CarbonSoilsTypes
      ] as number;
      const second = b[
        sortDescriptor.column as keyof CarbonSoilsTypes
      ] as number;
  ...
  const renderCell = useCallback((item: CarbonSoilsTypes, columnKey: Key) => {
    const cellValue = item[columnKey as keyof CarbonSoilsTypes];
    switch (columnKey) {
      case "id":
        return (
