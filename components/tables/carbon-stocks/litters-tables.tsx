...
const columns: ColumnProps[] = [
  ...
  { name: "SAMPLE WET", uid: "sampleWet" },
  { name: "TOTAL SAMPLE DRY MASSAL", uid: "totalSampleDryMassal" },
];
...
export default function LittersTables({
  params,
  page,
  setPage,
  limit,
  ...
  // data-table-with-api
  const { fetch, data, meta, fetching } = useCarbonLittersApi();
  const [dataTables, setdataTables] = useState<CarbonLittersTypes[]>([]);
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a: CarbonLittersTypes, b: CarbonLittersTypes) => {
      const first = a[
        sortDescriptor.column as keyof CarbonLittersTypes
      ] as number;
      const second = b[
        sortDescriptor.column as keyof CarbonLittersTypes
      ] as number;
  ...
  const renderCell = useCallback((item: CarbonLittersTypes, columnKey: Key) => {
    const cellValue = item[columnKey as keyof CarbonLittersTypes];
